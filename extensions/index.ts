...
function compactSessionName(
	value: string,
	minWords = SESSION_WORD_MIN,
): string | undefined {
...
function parseGeneratedNames(value: string): {
	window?: string;
	session?: string;
} {
	const lines = value
...
function buildConversationNamingSource(
...
async function renameCurrentTmuxWindow(
	...
	try {
		const result = await pi.exec(
			"tmux",
			buildRenameWindowArgs(name, targetWindow),
		);
...
export default function tmuxWindowNameExtension(pi: ExtensionAPI) {
	...
	const captureTmuxWindowTarget = async (): Promise<string | undefined> => {
		...
		const work = resolveTmuxWindowTarget((command, args) =>
	...
	const runRename = async (
		...
		if (
			!force &&
			(hasNameForSession || hasAttemptedNameForSession || renameInFlight)
		) {
	...
	const restoreExistingSessionName = async (ctx: ExtensionContext) => {
		...
		const windowName =
			storedWindow ?? compactWindowName(existing, 1) ?? existing;
		await renameCurrentTmuxWindow(pi, windowName, targetWindow);
		hasNameForSession = true;
		hasAttemptedNameForSession = true;
	};

	const renameFromBranch = async (
		args: string,
		ctx: ExtensionCommandContext,
	) => {
		const explicitName = compactWindowName(args, 1);
		...
		if (explicitName) {
			const targetWindow = await captureTmuxWindowTarget();
			const existingSessionName = pi.getSessionName();
			const sessionName = existingSessionName ?? explicitName;

			await pi.setSessionName(sessionName);
			await pi.appendEntry(WINDOW_NAME_ENTRY_TYPE, {
				windowName: explicitName,
			});
			await renameCurrentTmuxWindow(pi, explicitName, targetWindow);
			hasNameForSession = true;
			hasAttemptedNameForSession = true;
			notify(ctx, `Renamed window: ${explicitName}`, "info");
			return;
		}

		const conversation = buildConversationNamingSource(
			ctx.sessionManager.getBranch(),
		);
		const result = await runRename(conversation, "conversation", ctx, {
			force: true,
		});
	...
	pi.registerCommand("rename", {
		description:
			"Rename the current session from user and assistant messages in this branch",
	...
	pi.on("before_agent_start", async (event, ctx) => {
		const firstPrompt =
			getFirstUserPrompt(ctx.sessionManager.getBranch()) ?? event.prompt;
